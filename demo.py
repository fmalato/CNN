...
optimizer = tf.train.AdamOptimizer(learning_rate=1e-6).minimize(cost) ## Original: 1e-4
optimizer2 = tf.train.GradientDescentOptimizer(learning_rate=1e-8).minimize(cost) ## Original: 1e-2
correct_prediction = tf.equal(y_pred_cls, y_true_cls)
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

def show_progress(epoch, feed_dict_train, feed_dict_validate, valLosses):
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        show_progress(epoch, feed_dict_tr, feed_dict_val, valLosses)
...
session.run(tf.global_variables_initializer())
valLosses = train(30) # 250 IS ENOUGH

plt.plot(range(0, total_iterations), valLosses)
plt.xlabel("Iteration")
plt.ylabel("Validation loss")
plt.show()
""" Need to give weights and biases back to retrain the model on raspberry """
